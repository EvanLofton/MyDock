...
换个方向就没有这个问题：**修桌面那一边**。`reveal` 主循环每 2 秒扫一次 Z 序
...
同时留一行日志：`[层级] 桌面窗口跑到了 Dock 上面（多半是 explorer 重启）—— 已把桌面沉回 Dock 下方`。
...
（`ex=0x8000190` 从头到尾没变），自检 `drive_layering_test` 6 项通过 —— 关键那条断言的是
"**当场**就没生效"，而不是"过一会儿被纠正"。
